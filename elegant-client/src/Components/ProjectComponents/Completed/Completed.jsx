...
const Completed = () => {
  ...
  const completedConstructionData = constructionDatas
    ? constructionDatas.filter(({ status }) => status === "Completed")
    : [];

  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-10 mt-20">
      {completedConstructionData.map(({ id, name, mainImage, address }) => (
        <div
          key={id}
